...
function UnBotGetCostEnergyText(costType, costValue)
	...
	if (costType == 0) then
		return "消耗"..tostring(costValue).."法力"; -- Needs correction
	elseif (costType == 1) then
		return "消耗"..tostring(costValue).."怒气"; -- Needs correction
	elseif (costType == 3) then
		return "消耗"..tostring(costValue).."能量"; -- Needs correction
	else
		return "消耗"..tostring(costValue).."资源"; -- Needs correction
	end
end
	--item [2] = spellID
	--item [3] = name
			...
			if (needQuery == false) then
				if (fromParent.bagsType == 1) then
					...
					if (fromParent.dataGroup[newFrame.dataIndex][7] ~= nil and fromParent.dataGroup[newFrame.dataIndex][7] > 1) then
						...
						GameTooltip:AddDoubleLine("拥有数量：",tostring(fromParent.dataGroup[newFrame.dataIndex][7]),0,0.8,0.8,0.8,0.8,0); -- Needs correction
					end
				elseif (fromParent.bagsType == 2) then
					...
					if (spellLink ~= nil) then
						...
						if (spellData[9] <= 0) then
							...
							GameTooltip:AddDoubleLine("施法时间: " .. tostring(spellData[9]/1000).." 秒施法",castDis,0.65,0.55,0,0,0.8,0.8); -- Needs correction
						end
					end
				end
			else
				GameTooltip:AddLine(fromParent.dataGroup[newFrame.dataIndex][3],1,0,0,1);
				GameTooltip:AddLine("该道具没有在你的背包出现过，需要查询服务器，请点击界面下方的刷新按钮重新显示。",1,0,0,1); -- Needs correction
			end
			GameTooltip:AddLine(" ",1,1,1,1);
			if (fromParent.command ~= nil and fromParent.command ~= "") then
				if (fromParent.bagsType == 1) then
					if (fromParent.command == UnBotExecuteCommand[66]) then
						GameTooltip:AddLine("鼠标左键：装备",0.65,0.55,0,1);
					elseif (tostring(fromParent.command) == UnBotExecuteCommand[65]) then
						GameTooltip:AddLine("鼠标左键：丢弃",0.65,0.55,0,1);
					elseif (tostring(fromParent.command) == UnBotExecuteCommand[67]) then
						GameTooltip:AddLine("鼠标左键：出售",0.65,0.55,0,1);
					elseif (tostring(fromParent.command) == UnBotExecuteCommand[68]) then
						GameTooltip:AddLine("鼠标左键：使用",0.65,0.55,0,1);
					else
						GameTooltip:AddLine("鼠标左键：使用",0.65,0.55,0,1);
					end
				elseif (fromParent.bagsType == 2) then
					GameTooltip:AddLine("鼠标左键：施法",0.65,0.55,0,1);
				else
					GameTooltip:AddLine("鼠标左键单击：让 "..fromParent.target.." "..fromParent.activeText,0.65,0.55,0,1); -- Needs correction
				end
			end
			GameTooltip:AddLine("鼠标右键：隐藏",0.65,0.55,0,1);
