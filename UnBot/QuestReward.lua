...
function ShowQuestReward(name, items)
	...
	if (qrLabel ~= nil) then
		qrLabel:SetText(name.." |cffcccc00完成任务选择奖励|r"); -- Needs correction
	else
		DisplayInfomation("完成任务奖励面板Label "..qrFrame:GetName().."TitleLabel 未找到，设置玩家名失败 "..name); -- Needs correction
	end
...
function UnBotShowQuestRewardTips(self, item)
	...
	if (item[2] == true) then
		...
		if (itemName ~= nil and texture ~= nil) then
			...
			GameTooltip:AddLine("该道具没有在你的客户端出现过，需要等待片刻查询服务器后再次将鼠标移动到这个道具上。",1,0,0,1); -- Needs correction
		end
	else
		GameTooltip:SetHyperlink("item:"..tostring(item[1])..":0:0:0:0:0:0:0");
	end
